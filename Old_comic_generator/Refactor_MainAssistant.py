### Import Sysyem libraries
from Refactor_MainImports import *
# Only the imported elements will be available

### Import Custom libraries
from Refactor_SampleCustomImports import *
# Add self-defined libs to customize new layer 

### Import system parameters
from Refactor_SettingParameters import *
# Set width, height, default panels numbers, etc..
...
class MainApp(App):
    ...
    def open_file_chooser(self, instance):
        ...
        file_chooser = FileChooserIconView(filters=['*.png', '*.jpg', '*.jpeg'], size_hint=(1, 0.8))

        # Create a popup to hold the file chooser
        ...
        # Bind the select button to the load image function
    ...
    def load_image(self, selection, popup):
        ...
        if selection:
            ...
            # Load the image in the image widget
            self.scene_input_2.source = image_path
            self.scene_input_2.reload()

        # Dismiss the popup
        popup.dismiss()

    def draw_whole(self, resultSequence, inner_back):
        print("System: Clear all panels")
        inner_back.clear_widgets()
        print("System: ReDraw panels based on semantic graph!")
    ...
    def build(self):
        ...
        index = 2
        self.scene_input_2 = PanelLayout(size=(input_width, input_height), pos = (menu_count*input_w + width_shift, input_top - vertical_shift * index), size_hint=(None, None))
        # panelFrameList[str(panelIndex)] = new_panel_layout
        input_back_2.add_widget(self.scene_input_2)

        import_button_scene_L2 = Button(text ='Import', size =(input_button_w, input_button_h), pos = (menu_count*input_w + width_shift, input_top - vertical_shift * index - button_height_shift), background_normal ="", background_color =(37/255, 100/255, 77/255, 1), size_hint=(None, None), on_press=self.open_file_chooser)
        ...
        self.scene_input_2.add_widget(new_chara_image)
        ...
        reload_button = Button(text ='Generate', size =(reload_w, reload_h), pos = (new_position_w + (DEFAULT_MENU_WIDTH-(reload_w))/2, 50), background_normal ="", background_color =(156/255, 207/255, 83/255, 1), size_hint=(None, None), on_press=lambda x: self.draw_whole(resultSequence, inner_back))
        ...
        self.draw_whole(resultSequence, inner_back)
        # panelFrameList = {}
        # panelIndex = 0
